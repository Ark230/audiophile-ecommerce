...
const Description = props => {
    const {isTitleVisible, productDetailPage, variant, productName, productDescription} = props;
    const classes = useStyles();
    
    return (
        <div className={classes.descriptionContainer}>
            { isTitleVisible && <span className={classes.newProduct}>NEW PRODUCT</span>}
        <h1 className={classes.productTitle}>{(new String(productName)).toUpperCase()}</h1>
